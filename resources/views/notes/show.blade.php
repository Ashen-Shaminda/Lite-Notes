            ...
            <div class="flex text-white">
                @if (!$note->trashed())
                    ...
                    <form action="{{ route('notes.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-link btn-danger ml-4"
                            onclick="return confirm('Are you sure to move this to trash ?')">Move to
                            Trash</button>
                    </form>
                @else
                    ...
                    <form action="{{ route('trashed.destroy', $note) }}" method="post">
                        ...
                        @csrf
                        <button type="submit" class="btn btn-link btn-danger ml-4"
                            onclick="return confirm('Are you sure, you wish to delete this note permanetly?')">Delete
                            Forever</button>
                    </form>
                @endif
            </div>
